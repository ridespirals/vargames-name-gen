...
// WithRetries sets max retries (default 3).
func WithRetries(n int) ClientOption {
	return func(c *Client) { c.retries = n }
}

// WithBackoff sets min and max backoff for retries.
...
// endpoint is the path segment only (e.g. "games"); base URL is taken from config.
